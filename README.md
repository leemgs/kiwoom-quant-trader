...
3. **투자 한도 설정**: `.env`의 `MAX_TRADING_LIMIT` (또는 `INVESTMENT_BUDGET`) 항목을 통해 자동매매에 사용할 총액을 제한할 수 있습니다. (예: 계좌에 100만원이 있어도 50만원만 사용하도록 설정 가능)
...
```env
MAX_TRADING_LIMIT=10000        # 초기 자본금 전액 활용
